...
| `tests/` | the simulator against brute force, the box types against a voxel grid, and the duals against their closed forms |

### The environment

The state is the PCT triple: the packed items `C_t` (position, size and type
each), the observable conveyor `B_t` (sizes and types), and the feasible
positions `L_t` generated for whichever item is at the front, each carrying the
type of whatever it would rest on. An action picks one `l ∈ L_t`.
...
Every box carries a discrete `type_id`, and the type is the SKU: `types:` in
`config.yaml` gives one item-size class per type, so drawing a box is drawing a
type and then its sides from that type's own bounds. The rule is that **a box
may rest only on boxes of its own type**; the bin floor takes any type.

What "rest on" means is the whole content of the rule. The environment keeps a
type map `tmap[x, y]` beside the height map — the type of the topmost box in
each column — and for an incoming type `T` builds

    hbad[x, y] = hmap[x, y]   where the column is topped by some other type
               = −1           on bare floor, or under a box of type T

A footprint is legal exactly when the window maximum of `hbad` over it is below
the height `z` the item lands at. Every cell the item *touches* attains `z`, and
every cell below `z` is a void it bridges, so an overhang hanging over a foreign
type is legal and a contact patch on one is not. It is the same running-window
...
same window maximum carries the landing height in its quotient and the type
underneath in its remainder.

The rule costs one sweep pair per step over the untyped simulator. The station
filter is the expensive half — one feasibility sweep per *reachable* box, so at
`--n_pick 5` it roughly doubles the env step — which is why it is off unless it
can matter: `BPPBatch(pick_feasible=)` defaults to on only when there is more
than one type, the rule is on, and the reach is more than one. On the 30×25×40
pallet an `--algo select --n_pick 5` iteration runs at 9.4 s against 1.2 s for
`--algo exact` untyped, of which the pre-existing EMS enumeration is still the
largest single term.

With a reach of one there is nothing to choose and a blocked box ends the
episode, so `n_types > 1` and `n_pick > 1` belong together: with a reach the
station's `b_pick` hides the boxes that have nowhere left to go, and the
permuter — adversarial under `rarl`/`exact`/`approx`, cooperative under
`select` — is pointed at the ones it can actually hand over. The episode ends
when the whole station is blocked. `env.type_blocked()` reports the bins that
ended on the rule rather than on a full bin.

`--n_types 1` with `types: null` is the pre-type simulator exactly, and
`--type_constraint 0` keeps `type_id` in the state while letting anything stack
on anything.

**What the rule costs.** DBL on the 30×25×40 pallet, 128 bins, `min_support`
0.30:

| | Uti. | items |
|---|---|---|
| one type, no rule — the pre-type simulator | 61.1% | 40.1 |
| three types, rule off | 64.5% | 53.6 |
| three types, rule on, FIFO conveyor | 29.4% | 24.3 |

and on a FIFO conveyor **240 of 256 episodes end because the incoming box is
blocked by the rule, not because the pallet is full** (`env.type_blocked()`).
That is the whole shape of the problem the rule creates: with the types
interleaved at random, a heuristic that cannot choose its next box spends the
pallet on floor space and then stops. Substituting a naive selector — take the
reachable box with the most placements open — makes it *worse* (17.2% at reach
10), because "most placements open" means "the empty floor". Deciding which of
...
observable items `B_t` for the attacker and the mixture model. 70k parameters
...
Each encoder owns a trainable type embedding of `n_types + 1` rows — one per
box type, plus the bin floor — of width `model.type_embed` (16). It is
concatenated onto a node's physical features before the element-wise
projection, so `fc_in` is 16 columns wider and nothing downstream changes:

| node | features |
|---|---|
| packed item `C_t` | `[x, y, z, w, l, h]` + `emb(type_id)` → 22 |
| observable item `B_t` | `[w, l, h]` + `emb(type_id)` → 19 |
| candidate `L_t` | `[x, y, z, w, l, h]` + `emb(T_under)` → 22 |

The attacker and the mixture model read the same typed `B_t`, so what they
learn is a reordering over types as well as over shapes — which box is worst to
hand the packer next depends on what it is already stacking. A checkpoint
trained before types still loads: `evaluate.load_nets` pads the narrow
projections with zeros, which is the identity on the features the run was
trained with.
