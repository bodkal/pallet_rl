...
def profile(pack, attacker, nb, seqs, device="cuda", stability="com",
    ...
    while not env.done.all():
        ...
        # the sizes only: the type rides in column 3 and is profiled apart
        chosen.append(win[np.arange(len(seqs)), idx][alive][:, :3])
        kinds.append(win[np.arange(len(seqs)), idx][alive][:, 3])
